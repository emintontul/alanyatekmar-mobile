...
import {AdminBottomTabItem} from './BottomTabItem';
...
const FloatButton = ({onPress, isVisible}: {onPress: () => void; isVisible: boolean}) => {
  return <AdminBottomTabItem isFocused={isVisible} name={Routes.INVOICES_MAIN} onPress={onPress} />;
};

export const AdminBottomTabContainer = (props: BottomTabBarProps) => {
      ...
      <Shadow
        ...
        {state.routes.map((route, index) => {
          ...
          return route.name === Routes.INVOICES_MAIN ? (
            ...
            <AdminBottomTabItem isFocused={isFocused && !floatMenu} key={index} name={route.name} onPress={onPress} routesLength={state.routes?.length} currentIndex={index}  />
          );
        })}
        </Block>
      </Shadow>
